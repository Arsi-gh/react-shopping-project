    ...
    <div className="flex p-2 gap-4 h-full text-neutral-700">
        <div className="flex flex-col p-3 gap-2 rounded-lg bg-gradient-to-b from-zinc-200 to-white text-neutral-800 h-[80%] overflow-auto">
            <div className="flex items-center gap-2">
                <input placeholder="Search code ..." type="text" className="p-2 px-4 shadow-customeTwo rounded-lg flex-1"/>
                <MagnifyingGlassIcon className="cursor-pointer w-[2rem] p-2 rounded-lg bg-green-500 text-white"/>
            </div>
        ...
        <div className="flex flex-wrap content-start flex-1 gap-3 bg-gradient-to-b from-zinc-200 to-white rounded-md p-4 h-fit">
            ...
            <label className="w-[49%] p-2 flex gap-2 items-center" htmlFor="code-date-expire">
            ...
            <button className="p-2  px-[2rem] rounded-lg bg-green-500 text-white font-bold">Create</button>
        </div>
    </div>
  )
}
...
const OffCodeItem = () => {
    ...
    return (
        <div className="w-[20rem] flex flex-col p-1 rounded-lg bg-white shadow-customeTwo text-neutral-800">
            <p className="flex items-center justify-between p-2"><b className="flex gap-2 items-center"><TagIcon className="w-[1.5rem]"/> #Summer1400 </b><span onClick={e => setIsOpen(!isOpen)} className="cursor-pointer">{isOpen ? <EyeSlashIcon className="w-[1.6rem]"/> :  <EyeIcon className="w-[1.6rem]"/>}</span></p>
            {isOpen && 
                <>
                    <span className="w-full h-[1.5px] bg-zinc-200"></span>
                    <p className="flex items-center justify-between p-2 "><b className="flex gap-2 items-center"><CalendarDaysIcon className="w-[1.5rem]"/> Expire date : </b>23 - April - 2023</p>
                    <p className="flex items-center justify-between p-2 "><b className="flex gap-2 items-center"><UserGroupIcon className="w-[1.5rem]"/> For : </b> All users</p>
